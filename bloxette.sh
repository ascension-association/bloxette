...
if [ "$1" = "update" ]; then
  ...
  touch /tmp/bloxette/geo-ipv4 /tmp/bloxette/geo-ipv6
  if [ -f /var/lib/bloxette/geo.txt ]; then
    while IFS="" read -r line || [ -n "$line" ]; do
      if [[ "$line" =~ ^[A-Za-z]{2}$ ]]; then
        curl -sL "https://git.herrbischoff.com/country-ip-blocks-alternative/plain/ipv4/${line,,}.netset" >>/tmp/bloxette/geo-ipv4
        curl -sL "https://git.herrbischoff.com/country-ip-blocks-alternative/plain/ipv6/${line,,}.netset" >>/tmp/bloxette/geo-ipv6
      else
        continue
      fi
    done </var/lib/bloxette/geo.txt
  fi
  if [ -f /tmp/bloxette/geo-ipv6 ]; then
    cidr-merger -s /tmp/bloxette/geo-ipv6 >/tmp/bloxette/geo-ipv6-cidr
  fi
  if [ -f /tmp/bloxette/geo-ipv4 ]; then
    cidr-merger -s /tmp/bloxette/geo-ipv4 >/tmp/bloxette/geo-ipv4-cidr
  fi
  if [ -f /tmp/bloxette/geo-ipv6-cidr ]; then
    while IFS="" read -r line || [ -n "$line" ]; do
      echo "add bloxette-geo6 $line" >>/tmp/bloxette/geo-ipv6-ipset
    done </tmp/bloxette/geo-ipv6-cidr
  fi
  sudo ip6tables -D INPUT -m set --match-set bloxette-geo6 dst -j DROP >/dev/null 2>&1
  sudo ip6tables -D FORWARD -m set --match-set bloxette-geo6 dst -j DROP >/dev/null 2>&1
  sudo ipset destroy bloxette-geo6 >/dev/null 2>&1
  if [ -f /tmp/bloxette/geo-ipv6-ipset ]; then
    sudo ipset create bloxette-geo6 hash:net family inet6 hashsize 131072 maxelem 10000000 timeout 0
    sudo ipset restore </tmp/bloxette/geo-ipv6-ipset
    sudo ip6tables -A INPUT -m set --match-set bloxette-geo6 dst -j DROP
    sudo ip6tables -A FORWARD -m set --match-set bloxette-geo6 dst -j DROP
  fi
  if [ -f /tmp/bloxette/geo-ipv4-cidr ]; then
    while IFS="" read -r line || [ -n "$line" ]; do
      echo "add bloxette-geo4 $line" >>/tmp/bloxette/geo-ipv4-ipset
    done </tmp/bloxette/geo-ipv4-cidr
  fi
  sudo iptables -D INPUT -m set --match-set bloxette-geo4 dst -j DROP >/dev/null 2>&1
  sudo iptables -D FORWARD -m set --match-set bloxette-geo4 dst -j DROP >/dev/null 2>&1
  sudo ipset destroy bloxette-geo4 >/dev/null 2>&1
  if [ -f /tmp/bloxette/geo-ipv4-ipset ]; then
    sudo ipset create bloxette-geo4 hash:net family inet hashsize 131072 maxelem 10000000 timeout 0
    sudo ipset restore </tmp/bloxette/geo-ipv4-ipset
    sudo iptables -A INPUT -m set --match-set bloxette-geo4 dst -j DROP
    sudo iptables -A FORWARD -m set --match-set bloxette-geo4 dst -j DROP
  fi
fi
